...
if ( ! class_exists( 'WPS\WP\Taxonomies\SingleTermTaxonomy' ) ) {
	 ...
	 * Usage:
	 *
	 * $custom_tax_mb = new SingleTermTaxonomy( 'custom-tax-slug', array( 'post_type' ), 'type' ); // 'type' can be 'radio' or 'select' (default: radio)
	 *
	...
	class SingleTermTaxonomy {
		...
		public function select_open() {
			printf(
				'<select style="display:block;width:100%;margin-top:12px;" name="%s" id="%schecklist" class="form-no-clear">',
				$this->namefield,
				$this->slug
			);
			if ( ! $this->force_selection ) {
				printf(
					...
					esc_html( apply_filters( 'taxonomy_single_term_select_none', __( 'None' ) ) )
				);
			}
		}
		...
		public function radio_open() {
			printf(
				'<ul id="%1$schecklist" data-wp-lists="list:%1$s" class="categorychecklist form-no-clear">',
				$this->slug
			);

			if ( ! $this->force_selection ) {
				printf(
					'<li style="display:none;"><input id="taxonomy-%s-clear" type="radio" name="%s" value="0"/></li>',
					$this->slug,
					$this->namefield
				);
		...
		public function radio_close() {
			printf(
				'</ul><p style="margin-bottom:0;float:left;width:50%;"><a class="button" id="taxonomy-%s-trigger-clear" href="#">%s</a></p>',
				$this->slug,
				__( 'Clear' )
			);
			...
			printf(
				'jQuery(document).ready(function ($) {$(\'#taxonomy-%1$s-trigger-clear\').click(function () {$(\'#taxonomy-%1$s input:checked\').prop(\'checked\', false);$(\'#taxonomy-%1$s-clear\').prop(\'checked\', true);return false;});});',
				$this->slug
			);
		...
		public function terms_adder_button() {
            ...
            <p style="margin-bottom:0;float:right;width:50%;text-align:right;">
                <a class="button-secondary" id="taxonomy-<?php echo $this->slug; ?>-new"
                   href="#"<?php if ( 'radio' == $this->input_element ) : ?> style="display:inline-block;margin-top:0.4em;"<?php endif; ?>><?php _e( 'Add New' ); ?></a>
            </p>
		...
		/**
		 * Gets the SingleTermTaxonomy_Walker object for use in term_fields_list and ajax_add_term
		 * @since 0.2.0
		 * @return object SingleTermTaxonomy_Walker object
		 */
		 ...
		 *
		 * @return SingleTermTaxonomy  Returns SingleTermTaxonomy object, allows for chaining.
		 */
